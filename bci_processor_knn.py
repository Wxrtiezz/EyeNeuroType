# -*- coding: utf-8 -*-
"""
=== FILE: bci_processor_knn.py ===
NeuroType - BCI Processor using EOG + EEG features and KNN classifier.
"""
...
class BCIProcessor:
    def __init__(self, ip="192.168.1.198", k=3):
        self.ip = ip
    ...
    def load_model(self):
        ...
        if os.path.exists(model_path):
            try:
                ...
                print("[BCI] Đã tải model KNN và bộ chuẩn hóa Scaler thành công.")
            except Exception as e:
                print(f"[BCI] Lỗi tải model KNN: {e}")
        else:
            print("[BCI] Không tìm thấy bci_model_knn.pkl. Hệ thống sẽ dùng quy tắc ngưỡng cơ bản.")
    ...
    def classify(self, features):
        ...
        if self.model_loaded and self.model and self.scaler:
            try:
                ...
                # Tính độ tin cậy tỷ lệ lân cận biểu quyết của KNN
                try:
                ...
                print(f"[BCI] Lỗi phân loại KNN: {e}")
    ...
    def on_error(self, ws, error):
        print(f"[BCI] Lỗi kết nối WebSocket (KNN): {error}")

    def on_close(self, ws, close_status_code, close_msg):
        print("[BCI] Đã đóng kết nối WebSocket (KNN).")

    def on_open(self, ws):
        print(f"[BCI] Kết nối thành công đến ESP32 ({self.ip}) cho mô hình KNN!")
...
def start_websocket(ip="192.168.1.198", k=3):
    """Khởi động WebSocket Client trong một Thread nền"""
    processor = BCIProcessor(ip, k)
    
    def thread_worker():
        while True:
            try:
                print(f"[BCI] Đang kết nối đến ESP32 tại {processor.ws_url} cho bộ giải mã KNN...")
                processor.connect()
            except Exception as e:
                print(f"[BCI] Lỗi kết nối lại KNN: {e}")
            time.sleep(3)
